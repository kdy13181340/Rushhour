...
- **테마길 합본**: 공공자료 4종 8,053개 노선(16개 시도). 공원·하천변·등산로가 여기 있다(석촌호수 1,660그루 등).
  `GET /spots?theme=벚꽃&sido=서울` 목록 · `GET /spots/points` 좌표 있는 것 전부(8,016개).
  지도 우상단 **'나무 표시'** 토글로 켠다. 좌표가 노선당 한 점이라 **선이 아니라 점**으로 찍는다.
  그루수는 자료마다 조사가 달라 출처를 함께 보고, 미기재는 0이 아니라 '미기재'로 적는다(DP24).
