...
- 고른 것: **OSM 표준 타일 + CSS 회색 필터**. 색이 진해 경로 선을 덮으므로
  `.leaflet-tile-pane { filter: grayscale(1) brightness(1.06) contrast(0.82) }`로 밑그림만 누른다.
  필터가 tilePane에만 걸려 경로 선·마커 색은 그대로 — 시안의 '연한 바탕, 선명한 경로'가 유지된다.
  도보 앱이라 z17~19에서 골목·보도가 보이는 게 기능의 전제라 이 교체는 선택이 아니었다.
