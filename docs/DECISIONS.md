...
- 아직 안 한 것: **에이전트 그래프에 배선하지 않았다**. researcher는 여전히 `find_theme_streets`만
  부른다. 배선하면 답변이 출처를 구분해 말해야 한다("가로수 대장 445그루 / 단풍길 자료 1,660그루") —
  근거가 다른 숫자를 한 문장에 섞으면 안 된다. 다음 작업.
