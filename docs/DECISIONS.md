...
- 문서 단위: (구, 노선) 1,780건(5그루 미만 제외). 텍스트 = "서울 {구} {노선}. 동네: … 가로수 n그루: 수종별 …
  테마: 라벨(n그루; themes.py keywords) … 주의: 은행 열매 밟지 않는 길 테마에서 피하는 길(n그루). 인근 도로: …".
...
## DP24 — 공원·하천 산책로를 데이터에 들이다 (2026-09-12, BE)
- 문제: 가로수 대장은 **도로변에 심은 나무**만 센다(가로수 대장이니 당연하다). 그래서 사람들이
  실제로 벚꽃 보러 가는 석촌호수 둘레길·양재천·안양천이 통째로 빠져 있었다. 팀에서 "차 도로
  기준이라 명소가 없다"고 본 것의 진짜 원인이다.
- 확인한 것: 명소가 다 빠진 건 아니었다. 여의서로(윤중로)는 벚나무 445그루로 이미 들어 있다.
  빠진 건 **공원·하천 산책로**다(경의선숲길 0그루, 석촌호수는 둘레 도로의 93그루만).
- 팀이 넣어 준 자료 셋을 합쳐 `data/processed/seoul_theme_spots.parquet`를 만들었다
  (`scripts/07_build_theme_spots.py`):

  | 자료 | 무엇을 더해 주나 | 한계 |
  |---|---|---|
  | 가로수 대장(기존) | 1,607개 노선·28.2만 그루, 나무마다 좌표 | 도로변만 |
  | 전국 가로수길 표준데이터(서울) | 642개 노선·15.2만 그루, **시작·종료 좌표**·연장·소개 | 역시 가로(街路)만 |
  | 서울 단풍길 110선 | **공원 23·하천변 14·등산로 4**, 그루수·연장·선정사유 | **좌표 없음** |

- **합치되 더하지 않는다**: 같은 길이 자료마다 따로 조사돼 있고 그루수도 다르다(개포로: 대장
  1,004 / 표준 1,169). 984행이 다른 자료와 겹친다. 같은 (자치구, 노선명)에 `대표=True` 한 행만 두고
  나머지는 `중복출처`로 표시했다. 총계는 대표 행만 센다 — 단순 합산은 부풀린 숫자다.
- 결과(대표 행 1,842개 노선): 벚꽃 490개 노선 10.9만 그루(전 32,966), 은행단풍 911개 19.9만,
  그늘 877개 19.9만, 이팝 364개 6.6만, 메타세쿼이아 57개 1.5만.
  찾던 것들이 들어왔다 — 송파나루 근린공원(석촌호수) 1,660그루 · 안양천 산책로 2,277 ·
  송정제방길 5,000 · 양재천 738 · 여의도 운동길 1,736 · 남산공원 순환로.
- **좌표를 지어내지 않았다**: 단풍길 110선은 좌표가 없어 노선명을 표준데이터·가로수 데이터·OSM
  이름에 맞춰 봤다(96개 중 52개 성공). 못 맞춘 44개는 `위도/경도`를 비워 두고 `매칭=''`으로 남겼다 —
  지도에는 못 올려도 "어디에 몇 그루"는 답할 수 있다. 이름 매칭은 **꼬리만 다른 경우**만 받는다:
  느슨한 부분일치를 썼더니 '송정제방길'이 '송정10길'에 붙었다. 자치구가 어긋나는 매칭도 버린다.
- **koo6312.github.io/Cherry는 쓰지 않았다**: 그 사이트의 출처가 'OA-1325 가로수 위치정보'인데,
  팀이 받은 그 파일은 **중구 한 곳 7,745그루**뿐이다(공원·사유지 수목도 중구만 2,203그루).
  우리 가로수 대장은 25개 구 28만 그루에 벚나무 32,966그루가 좌표까지 있어 이미 더 낫다.
  남의 사이트를 긁는 대신 같은 원본 공공데이터에서 직접 만들었다 — 출처가 분명하고 갱신도 된다.
### DP24 이어서 — 전국으로 넓히고 지도에 얹다 (2026-09-12)
- 자료를 넷으로 늘리고 표준데이터를 **전국**으로 열었다. `data/processed/theme_spots.parquet`:
  대표 7,265개 노선(서울 1,923 · 그 외 5,342), 15개 시도. 벚꽃만 2,459개 노선.
- **지역 경계를 분명히 했다**: `지역구분` 컬럼으로 서울/전국을 가른다. 목록·지도 표시는 전국이
  되지만 **경로 찾기(plan_route)는 서울만** 된다 — 보행 도로망을 서울 bbox로만 받았기 때문이다.
  도구 docstring과 `/health`에 그대로 적어 뒀다.
- **연장(km) 단위 오류를 그 행 자신의 좌표로 검산해 고쳤다**: 여러 지자체가 미터로 적어 놨다
  (부평구 길주로 3,400 = 3.4km, 경산시 72,400km). 시작~종료 직선거리와 1000배 어긋나면 미터로 보고
  나눈다 — 838행 보정. 판별 못 한 326행은 **비웠다**. 단일 도로 72,400km가 남으면 "10km 코스"
  같은 답이 거짓이 된다. 지어낸 게 아니라 행이 이미 가진 좌표로 검산한 것이다.
- **지도에는 선이 아니라 점으로 찍는다**: 표준데이터의 좌표는 노선당 시작·종료 두 점뿐이다.
  두 점을 이으면 실제 도로를 따라간 경로처럼 보여 거짓이 된다(강변북로 6.6km를 직선으로 그으면
  한강을 가로지른다). 원의 크기가 그루수, 색이 구분(가로·공원·하천변·등산로)이다.
- 배선: `app/spots.py`(`find_theme_spots` @tool + `find_spots`), `GET /spots`, `/health`의 `spots`,
  지도 우상단 '명소 표시' 토글. 확인 — 전국 벚꽃 788곳·서울 477곳이 찍히고 하천변이 파랗게 구분된다.
- **데이터 손실 두 건을 잡았다**(둘 다 실측으로 발견):
  ① 제주 154개 노선이 통째로 빠져 있었다 — 그루수가 전부 미기재라 `fillna(0) >= 10` 필터에 걸렸다.
     **모르는 것과 적은 것은 다르다**. 그루수를 nullable로 바꿔 미기재는 `<NA>`로 남기고, 답변·화면에
     "그루수 미기재"로 정직하게 적는다(0으로 바꾸면 '나무가 없다'는 거짓말이 된다). 912개 노선이 해당.
     왕벚나무 원산지인 제주가 빠진 채 '전국'이라 할 뻔했다.
  ② 이름 매칭만 하고 좌표를 안 가져와, 석촌호수(송파나루 근린공원) **1,660그루가 지도에 못 올라갔다**.
     OSM 매칭 시 그 이름을 가진 간선들의 좌표 평균을 함께 담도록 고쳤다 — 37.5095, 127.1022로 잡힌다.
     단풍길 좌표 확보가 53→59개로 늘었다.
- 화면은 '명소 고르기'가 아니라 **좌표가 있는 것을 전부** 찍는다(`GET /spots/points`, 8,016개, 0.8초).
  그루수 상위만 추리면 전국 분포가 안 보인다. 전국 수천 개를 DOM 마커로 그리면 지도가 멈춰 캔버스로 그린다.
- **테마를 눌러도 전국이 안 나오던 것**을 고쳤다(제보로 발견): 왼쪽 목록의 '봄 벚꽃길'은
  `/ui/theme/{id}` → `find_theme_streets`를 타서 **서울 상위 6개 도로**만 보여 줬다. 합본은 토글에만
  붙어 있어 둘이 따로 놀았다. 이제 켜진 테마를 따라 합본도 그 테마로 좁혀 다시 그린다
  (벚꽃이면 전국 2,724곳). 토글은 **기본으로 켜 둔다** — 꺼 두면 전국 자료가 있는 줄 모른 채
  서울 6개 도로만 보게 된다.
- 곁가지: 경로 답변의 `key`는 shortest/theme/avoid라 그대로 테마 필터에 넘기면 결과가 0이 되어
  지도가 텅 빈다. 아는 테마일 때만 좁히도록 막았다.

### DP24 이어서 — 나무 그림으로, 구간을 따라 (2026-09-12)
- 제보 셋을 한 번에 고쳤다: ①동그라미 말고 나무 모양으로 ②토글은 빼고 물은 것만 ③석촌호수에
  벚나무가 여러 그루일 텐데 점 하나뿐.
- **①·②**: 합본도 `trees.js`의 수종별 나무 SVG로 그린다(벚꽃은 분홍 꽃잎). 토글을 없애고
  **켠 테마를 따라** 저절로 나온다 — 아무 테마도 안 켜면 나무를 그리지 않는다.
- **③이 진짜 문제였다**: 가로수 대장의 '석촌호수로 벚나무 93그루'는 **호수가 아니라 1.5km 서쪽**에
  몰려 있다(중심 37.514, 127.085). 호수 둘레 벚나무는 공원 수목이라 대장에 아예 없다. 합본의
  1,660그루(송파나루 근린공원)가 유일한 근거인데 좌표가 노선당 한 점뿐이라 점 하나로 찍혔다.
- 그래서 **구간을 따라 흩뿌린다**: 이름을 맞춘 길은 OSM 실제 형상(`routing.way_path`, 55/96),
  표준데이터는 시작~종료 두 점(5km 넘으면 직선이 실제 길과 크게 어긋나 안 씀). 그루수의 제곱근에
  비례해 최대 14그루까지 구간에 고르게 놓는다.
  **개별 나무 위치를 아는 게 아니라 '이 구간에 이만큼'이라는 표시**이고, 툴팁에 그루수와 출처를
  적는다. 팀 발표용이라 시각적 표현을 우선한다는 것이 팀 결정(2026-09-12) — 데이터의 그루수·출처는
  그대로 두고 **그리는 방식만** 바꾼 것이다.
- 상위 6개 도로만 주던 것도 고쳤다(`GET /map/theme_points`): 그 테마 가로수를 **전부** 내려
  화면에 보이는 것만 46px 격자로 솎아 그린다(벚꽃 31,550그루). 순위 밖 도로가 사라지지 않는다.
- 아직 안 한 것: **에이전트 그래프 배선**. researcher는 여전히 `find_theme_streets`만 부른다.
  서울 제한을 푸는 것은 **에이전트 담당(B)이 맡기로 팀에서 정했다**(2026-09-12). BE는 데이터와 도구·API·
  지도까지 대고, 배선할 때 답변이 출처를 구분해 말하게 하는 것은 그쪽에서 한다 —
  "가로수 대장 445그루 / 단풍길 자료 1,660그루"처럼 근거가 다른 숫자를 한 문장에 섞으면 안 된다.
...
## DP25 — 나무가 지도에 한 그루만 찍히던 문제 (2026-09-12, BE)

- **증상(사용자 보고)**: "클릭해 보면 63그루라는데 지도엔 나무가 하나뿐이라 너무 없어 보인다."
- **원인 둘**:
  1. 그릴 개수를 그루수만 보고 정했다 — `min(14, max(1, round(√c / 6)))`.
     63그루면 `√63/6 ≈ 1.3` → **1개**. 133그루(중앙값)도 2개였다.
  2. 합본 자료는 노선당 좌표가 **한 점**뿐이다. 구간을 몰라 흩뿌릴 수가 없었다.
- **고친 것**
  - **개수는 줌이 정한다**: 나무 그림 사이가 화면에서 늘 58px쯤 떨어지도록 미터 간격을 환산한다.
    확대하면 촘촘해지고 축소하면 노선당 한 그루가 된다. 아는 그루수보다 많이 그리지는 않는다.
  - **보이는 토막만 쓴다**: 3km짜리 길에 고르게 흩으면 확대했을 때 대부분 화면 밖이라
    눈앞의 길이 텅 빈다. 화면에 걸치는 구간으로 잘라 그 안에 몰아 준다.
  - **길 양옆으로 번갈아**(중심선에서 ±11m). 가로수는 길가에 줄지어 서 있다.
  - **형상을 더 붙였다**: `attach_shapes`를 합본 전체에 돌리고, 이름 매칭이 없는 행은
    행의 노선명으로 도로망을 찾되 **행 자신의 좌표에서 1.2km 안**인 것만 받는다.
    실제 도로 형상 **55 → 746개**. 거리 검증이 필요한 이유는 실측으로 나왔다 —
    이름은 같은데 딴 동네인 경우가 404건 중 163건이었다(‘중앙로’ 등이 전국에 겹친다).
  - **축소하면 격자로 고르게**: 화면에 수천 개가 걸릴 때만, 예산에 맞춘 격자를 깔고 칸마다 하나씩
    흩어진 순서로 채운다. 가까운 것부터 채우면 가운데만 뭉치고 가장자리가 빈다.
    다 들어가는 줌에서는 솎지 않는다(안 그러면 노선 둘뿐인 여의도가 네 그루가 됐다).
- **실측(1280×800 창, 브라우저와 같은 계산을 파이썬으로 재현)**

  | 화면 | 화면 안 노선 | 나무 그림 | 간격 | 화면 16칸 중 분포 |
  |---|---|---|---|---|
  | 석촌호수 z16 | 3 | **3 → 30** | 121m(=64px) | 6칸 |
  | 여의도 z15 | 11 | 27 | 258m(=68px) | 9칸 |
  | 서울 도심 z14 | 262 | 254 | 673m(=89px) | 16칸 |
  | 서울 도심 z12 | 1,023 | 260 | — | 16칸 |
  | 전국 z8 | 2,687 | 169 | — | 9칸(나머지는 바다·산) |

- **한계 — 분명히 해 둔다**: 이 점들은 **개별 나무의 실제 위치가 아니다**. 공공자료가 주는 것은
  '그 구간에 몇 그루'까지이고, 그림은 그 구간을 눈에 보이게 한 것이다. 툴팁에 그루수와 출처를
  그대로 적어 두는 이유다. 기반 데이터의 그루수·출처는 손대지 않았다.
- 발표용 시안이라 정확도보다 가독성을 택한 것이고, 사용자도 그 전제를 밝혔다.
