...
**BE 남은 것**:
- **`theme_spots`를 에이전트에 배선** — 도구(`find_theme_spots`)·API·지도까지는 됐지만(DP24)
  researcher가 아직 안 부른다. 답변이 출처를 구분해 말하도록 resolver도 같이 손봐야 함
- **경로를 전국으로 넓힐지** — 목록·지도는 전국인데 `plan_route`는 서울만(도로망 bbox). 넓히려면
  시도별 OSM을 더 받아야 한다(서울만 3분·237k 노드 기준)
...
| DP24 | 공원·하천 산책로·전국 | 공공자료 4종 합본(theme_spots) 7,265노선·15시도. 합치되 더하지 않음. 지도엔 점으로 |
