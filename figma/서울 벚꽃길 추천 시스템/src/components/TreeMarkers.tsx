...
// SVG tree illustrations per route type
const TREE_SVGS: Record<string, (size: number, seed: number) => string> = {
  cherry: (s, seed) => {
    const petal = (a: number, r: number) =>
    ...
    const petals = [0, 1, 2, 3, 4, 5, 6].map((i) => petal((i / 7) * Math.PI * 2 + seed * 0.3, s * 0.22)).join("");
...
function getTreeSvg(routeId: string, size: number, seed: number): string {
  const fn = TREE_SVGS[routeId];
  if (!fn) return TREE_SVGS["shade"](size, seed);
  return fn(size, seed);
}

function interpolatePath(path: [number, number][]): [number, number][] {
...
const OFFSET_LAT = [0.0002, -0.0003, 0.0001, -0.0002, 0.0003, -0.0001, 0.0002, -0.0002];
const OFFSET_LNG = [0.0003, 0.0001, -0.0002, 0.0002, -0.0003, 0.0001, -0.0001, 0.0003];

export default function TreeMarkers({ activeRoutes }: { activeRoutes: Route[] }) {
  return (
    <>
      {activeRoutes.map((route) =>
        route.paths.map((path, pi) => {
          const pts = interpolatePath(path as [number, number][]);
          return pts.map((pt, i) => {
            const seed = (pi * 31 + i * 7) % 10;
            const size = SIZE_VARIANTS[seed];
            const lat = pt[0] + OFFSET_LAT[seed % OFFSET_LAT.length];
            const lng = pt[1] + OFFSET_LNG[seed % OFFSET_LNG.length];
            const svgStr = getTreeSvg(route.id, size, seed);
            const svgWidth = route.id === "metasequoia" || route.id === "evergreen" ? size * 0.7 : size;
