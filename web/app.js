/* 서울 가로수길 — 프론트 로직.
   ...
   전부 /api/* 에서 받는다 — 폴리라인은 실제 나무 좌표에서 접은 중심선이다.
...
const SEASON_ORDER = ['spring', 'summer', 'fall', 'allseason'];
...
/* 켜진 경로는 state.active가 들고 있는 paths로 그린다. 에이전트가 자치구를
   집어낸 답변이면 그 paths가 구 단위로 좁혀져 온다(api.py theme_brief). */
function drawLines() {
...
/* ── 상태 전환 ────────────────────────────────────────────── */
/** briefs: /api/chat이 준 경로 객체들. 자치구로 좁혀진 paths·points가 실려 올 수 있다. */
async function setActive(briefs) {
  ...
  await Promise.all(state.active.map(async (r) => {
    ...
    try {
      ...
      const d = await (await fetch(`api/theme/${r.id}`)).json();
      r.points = d.points || [];
...
async function sendQuery(q) {
  ...
  try {
    ...
    wait.remove();
    bubble('assistant', r.answer || '(답변 없음)', r.routes);
    await setActive(r.routes);
  } catch (e) {
    wait.remove();
